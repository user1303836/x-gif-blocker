...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'COMPUTE_HASH' && message.target === 'offscreen') {
    console.log('[Offscreen] Received COMPUTE_HASH for:', message.url);
    computeHash(message.url).then(result => {
        console.log('[Offscreen] Computed hash:', result);
        sendResponse(result);
    });
    return true;
  }
...
async function computeHash(url) {
  try {
    ...
    const hash = bmvbhash(imageData, 16); // 16 bits for 256-bit hash
